/** @scratch /panels/5
 *
 * include::panels/terms_tophits.asciidoc[]
 */

/** @scratch /panels/terms_tophits/0
 *
 * == terms_tophits
 * Status: *Stable*
 *
...
function (angular, app, _, $, kbn) {
  ...
  var module = angular.module('kibana.panels.terms_tophits', []);
  app.useModule(module);

  module.controller('terms_tophits', function($scope, querySrv, dashboard, filterSrv, fields) {
    $scope.panelMeta = {
      modals : [
        {
          description: "Inspect",
    ...
    var _d = {
      /** @scratch /panels/terms_tophits/5
       * === Parameters
       *
       ...
       * === Parameters
       *
       * labelField:: The field which is used as label
       */
      labelField : '_type', 
      /** @scratch /panels/terms_tophits/5
            ...
            .aggregation(
              $scope.ejs.FilterAggregation('terms')
                ...
                .aggregation($scope.ejs.TermsAggregation('terms')
                  ...
                  .aggregation($scope.ejs.TopHitsAggregation('terms')
                    .size(1)
                    )                    
                )
              ).size(0);         
          // .agg($scope.ejs.FilterAggregation())
    ...
    $scope.build_search = function(term,negate) {
      if(_.isUndefined(term.meta)) {
        filterSrv.set({type:'terms',field:$scope.field,value:term.key,
  ...
  module.directive('termsTophitsChart', function(querySrv) {
    return {
      restrict: 'A',
...
// scope.results.aggregations.terms.terms.buckets[0].terms.hits.hits[0]._source.stack_trace
        function build_results() {
          ...
          _.each(scope.results.aggregations.terms.terms.buckets, function(v) {
            var slice;
            if(scope.panel.tmode === 'terms') {
              slice = { label : v.terms.hits.hits[0]._source[scope.panel.labelField], key: v.key, data : [[k,v.doc_count]], actions: true};
            }
            if(scope.panel.tmode === 'terms_stats') {
